...
- [ ] T012 Implement `src/mcpworks_api/services/api_server.py` backbone: server CRUD (`add` manual path, `list`, `describe` with redaction, `remove`, `update_settings`), encrypted stored-credential helpers (`set_api_credentials` via `core/encryption.encrypt_value`), `add_manual_endpoint`, `enable_endpoint`/`disable_endpoint` (depends T004–T008, T011)
...
- [ ] T014 Add `TOOL_SCOPES` entries and dispatch handlers (`_add_api_server`, `_list_api_servers`, `_describe_api_server`, `_remove_api_server`, `_update_api_settings`, `_set_api_credentials`, `_add_manual_endpoint`, `_enable_endpoint`, `_disable_endpoint`, `_configure_agent_api_access`) in `src/mcpworks_api/mcp/create_handler.py` (depends T012, T013)
...
- [ ] T025 [US2] Wire execution transient-env access to the proxy via `ExecutionContext`/`exec_token_registry` (or sandbox-creation registration) so values are reachable server-side without crossing the sandbox→proxy body — `src/mcpworks_api/core/exec_token_registry.py` and the 002 env-passthrough integration point (depends T024)
