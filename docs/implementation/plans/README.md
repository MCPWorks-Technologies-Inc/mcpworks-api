...
|------|---------|--------|------|-------------|
| `technical-architecture.md` | 2.0 | Active | `../specs/mcpworks-mcp-server-spec.md` | **PRIMARY PLAN** System architecture for mcpworks Infrastructure MCP server: technology stack (Python/FastAPI), 19 MCP tools, provider abstraction layer, SSE streaming, credit system, deployment flows, scalability design (1K→10K→100K customers), disaster recovery, acquirer technical fit analysis. |
